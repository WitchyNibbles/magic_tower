...
definition. These tests read the README itself, so none of the three can come back.

The two directory tests run the documented lines instead of matching their text.
What the README promises about ``~/.magic-tower`` is an outcome -- it is there, and
it is owner-only -- and any spelling that delivers that outcome is a correct README.
"""
...
# Where the README tells the second PC to keep the labeled sample: outside the
...
def _fenced_blocks() -> list[list[str]]:
    """Every fenced code block in the README, one list of folded commands per fence.
...
def test_the_documented_compose_copy_has_a_directory_to_copy_into(tmp_path) -> None:
    ...
    assert copies, "the README no longer documents the docker compose cp step"
...
def test_every_documented_way_of_writing_the_sample_leaves_its_directory_owner_only(tmp_path) -> None:
    ...
    assert blocks, "the README no longer documents how to write a labeled sample"
